...
def calculate_mahalanobis_distances(data, mean_vector, inv_cov_matrix):
    ...
    try:
        mahalanobis_distances = np.sqrt(
            np.sum(centered_data @ inv_cov_matrix * centered_data, axis=1)
        )
...
def calculate_similarity_matrix(data, k, scale_parameter, knn_method="method_B"):
    mahalanobis_distances = calculate_mahalanobis_distances(
        data, np.mean(data, axis=0), np.linalg.inv(np.cov(data, rowvar=False))
    )
    ...
    similarities = (
        np.exp(-0.5 * np.square(mahalanobis_distances / scale_parameter)) * is_neighbor
    )
    return similarities


def MDLSC_algorithm(
    data, num_clusters, k_neighbors=5, scale_parameter=1.0, knn_method="method_A"
):
    try:
        # Calculate the similarity matrix
        similarity_matrix = calculate_similarity_matrix(
            data, k_neighbors, scale_parameter, knn_method
        )
        ...
        regularized_laplacian_matrix = laplacian_matrix + regularization_term * np.eye(
        ...
        subset_data_head = data[
            :5, :
        ]  # Use the original data instead of subset_data_std
        ...
        clusters = np.zeros(
            len(data), dtype=int
        )  # Assign all points to a single cluster in case of an error
...
# Load your dataset
data = pd.read_csv(
    "/content/drive/MyDrive/newdataset_spectral_clustering.dat", delimiter="\s+"
)
...
# Visualize the results
plt.scatter(
    subset_data_std[:, 0], subset_data_std[:, 1], c=clusters, cmap="viridis", s=50
)
plt.title("Spectral Clustering Results")
plt.xlabel("Feature 1")
plt.ylabel("Feature 2")
...
plt.subplot(2, 2, 1)
scatter = plt.scatter(
    subset_data_std[:, 19], subset_data_std[:, 0], c=clusters, cmap="viridis", alpha=0.7
)
...
plt.subplot(2, 2, 2)
scatter = plt.scatter(
    subset_data_std[:, 19], subset_data_std[:, 9], c=clusters, cmap="viridis", alpha=0.7
)
...
plt.subplot(2, 2, 3)
scatter = plt.scatter(
    subset_data_std[:, 19], subset_data_std[:, 5], c=clusters, cmap="viridis", alpha=0.7
)
...
plt.subplot(2, 2, 4)
scatter = plt.scatter(
    subset_data_std[:, 19], subset_data_std[:, 1], c=clusters, cmap="viridis", alpha=0.7
)
...
# Save the plot
plt.savefig(
    os.path.join(output_directory, "assigned_clusters_aod500_eae440_subplot.jpg")
)
